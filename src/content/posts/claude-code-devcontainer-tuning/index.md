...
コンテナの中のClaude Codeの画面もMacの同じターミナルアプリに表示しているのに、コンテナの中には`TERM_PROGRAM`が無いので、「どの端末か分からない＝リンク非対応」と判定されていました。
...
これでリンクが出るようになったので、自作の起動コマンド`ccd`が実行している`devcontainer exec`にも、同じ`--remote-env FORCE_HYPERLINK=1`を足しました。
...
撮ったスクショをClaude Codeに貼り付けると、実際に入力されるのは画像そのものではなく、Mac上の画像ファイルのパスです（`/Users/.../CleanShot 2026-08-09 at 12.45.21.png`のような文字列）。  
MacのClaude Codeはそのパスのファイルを読めるので画像として扱えますが、コンテナの中にそのパスは存在しないので、開けない、という状況でした。
...
そこで、この手作業をコマンドにしてみました。クリップボードの画像を受け渡し用のディレクトリへ保存し、そのパスを表示するだけのコマンドです。  
ただ、手順を並べると「撮る → コマンドを打つ → 出てきたパスをClaudeに貼る」で、手作業の「撮る → コピーする → パスを貼る」と手数は同じでした。  
毎回3手かかることが変わらない為、採用には至りませんでした。
...
クラウド上の画像URLのホスト名を調べると、記事作成時点ではCDN（画像などを配信する共有のサーバー網。ここではAWSのCloudFront）から配信されていました。  
一方このfirewallは、起動時にドメイン名をIPに変換して、そのIPだけを通す方式です。  
CDNは名前解決で返ってくるIPが固定とは限らず、一般に変わることがあります。CleanShotの配信先も変わる可能性があり、変わるとfirewallを通らなくなって画像を取得できず、その都度コンテナの再起動が必要になります。  
そのため、この方法も却下しました。
...
貼り付けで入力されるMac上のパスがコンテナの中でもそのまま読めるので、Macのパスをコンテナのパスに書き換える必要も、ファイルをコピーする必要もありません。撮って貼るだけで済みます。
