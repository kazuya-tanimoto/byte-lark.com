...
これでリンクが出るようになったので、自作の起動コマンド`ccd`に組み込みました。

## スクリーンショットが渡せない

Claude Codeでは、クリップボードの画像をそのまま貼り付けて「このスクショのここが崩れてる」と相談できますが、コンテナ内ではこれができませんでした。  
貼り付けでは画像の実体ではなく、Macのファイルパスの文字列だけが貼り付けられるようです。コンテナからそのパスは見えないので開けない、という状況でした。

最初はリポジトリ内のgitignoreされたディレクトリへ画像を手でコピーして、そのパスを貼って読ませていました。  
Playwrightが出力用に使っている`.playwright-mcp/`の間借りです。  
...
クリップボードから貼り付けられるパスが、コンテナからも読めるため、読み替えもコピーも要りません。撮って貼るだけで済みます。
...
コンテナの中Claude Codeに同じような不便を感じている人は、ぜひ解消を試してみてください。この記事がその役に立てば嬉しいです。
