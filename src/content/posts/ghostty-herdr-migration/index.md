...
Claude Codeのようなコーディングエージェントを複数走らせる作業がどう変わったかと、日本語入力のハマりどころ2件、いま使っている設定を書きます。  
ターミナルマルチプレクサ自体の説明はしません。

## herdrとは

[herdr](https://herdr.dev/)は、tmuxのようにペインとタブを持つターミナルマルチプレクサです。Rust製で、いま使っているターミナルの中でそのまま動きます。  
...
一言でいうと、tmuxをAIエージェント向けに再構築したものです。
...
決め手は、エージェントの状態を設定なしで検知して見せてくれるところと、キーバインドがtmux互換で学習コストが低そうなところでした。tmuxの上位互換になりそうだと思って試してみました。  
実際に入れてみると、tmuxでやっていたことはそのままできて、そのうえで状態の可視化と通知が付いてきました。いいやん、となってそのまま定着しました。
...
私もこれに倣って、repoごとにspaceを分け、その中をタスクごとのタブに割って、1タスクに1エージェントを走らせています。一時的なコマンド実行はペイン分割で足します。
...
herdrではサイドバーのagentsに状態が並ぶので、見て回る必要がなくなりました。
...
見に行かなくても向こうから知らせてくるので、待ち時間に別の作業へ移りやすくなりました。
...
prefixキー（操作の合図として最初に押すキー）を使っている間だけ、入力ソースをASCIIに切り替える機能です。日本語IMEのままprefixを押して文字が化けるのを防いでくれるのですが、元の入力ソースに戻し損ねることがあります（[herdr #1221](https://github.com/herdrdev/herdr/issues/1221)、0.7.3時点で未修正）。  
...
乗り換えて変わったのは、tmuxでできていたことはそのままに、エージェントの状態が見えて通知が来るようになったことです。おかげで巡回がなくなり、待ち時間に別の作業へ移れるようになりました。  
