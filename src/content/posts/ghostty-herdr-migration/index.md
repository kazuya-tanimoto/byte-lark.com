...
alacritty + tmuxは2年半使っていましたが、いまはghostty + herdrが標準になっています。

想定読者は、Claude Codeのようなコーディングエージェントを複数走らせたい人と、tmuxを使っていて乗り換え先を探している人です。  
ターミナルマルチプレクサ自体の説明はしません。

## herdrとは

[herdr](https://herdr.dev/)は、tmuxのようにペインとタブを持つターミナルマルチプレクサです。  
Rust製で、いま使っているターミナルの中でそのまま動きます。  
tmuxとの違いは、左サイドバーにspaces（作業空間の一覧）とagents（動いているAIエージェントの一覧）が出ることです。  
...
alacritty + tmuxに困っていたわけではありません。  
[herdrの紹介記事](https://zenn.dev/studypocket/articles/herdr-ai-agent-multiplexer)をたまたま見つけて、tmuxの上位互換になりそうだと思って試したのが始まりです。  
決め手は2つでした。  
エージェントの状態を設定なしで検知して見せてくれることと、キーバインドがtmux互換で学習コストが低そうなことです。  
実際に入れてみると、tmuxでやっていたことはそのままできて、そのうえで状態の可視化と通知が付いてきました。  
そのまま定着した形です。
...
herdrの公式ドキュメントは、workspace（サイドバーの表示ではspaces）をrepoやタスクの単位で分けることを勧めています。  
私はこう使っています。

- repoごとにspaceを分ける
- repoの中はタスクごとにタブを分ける
- 1タスクに1エージェント。一時的なコマンド実行はペイン分割で足す

![普段の作業画面。左にspacesとagentsのサイドバー、中央にエージェントのセッション、右にコマンド実行用のペイン。この記事を書いているセッション自体が写っている](./herdr-overview.png)

tmux時代は1画面に4〜6ペインを開いていましたが、この型にしてからは1〜2ペインで足りています。  
...
herdrではサイドバーのagentsに状態が並ぶので、見れば分かります。

![agentsサイドバーの拡大。blocked（入力や承認の待ち）、idle（確認済み）、working（作業中）、done（完了・未確認）の4状態が並んでいる](./herdr-agents-sidebar.png)

### 通知で気づけるようになった

エージェントが入力や承認を待つ状態になったり、作業が終わったりすると、macOSの通知が来ます。  
見に行かなくても向こうから知らせてくる形になり、待ち時間に別の作業へ移りやすくなりました。
...
herdrは、いま使っているターミナルをそのまま使えます。  
実際、最初はalacritty + herdrで試していました。  
dotfilesのコミットログでは、alacrittyの起動をherdrに切り替えたのが最初で、その約2週間後にghosttyの起動をherdrに切り替えています。

ghosttyに替えた理由は、記録にも記憶にも残っていません。  
この記事を書くにあたって試し直したところ、alacrittyではエージェントの通知が音だけで、デスクトップ通知が出ませんでした。  
ghosttyだけを起動し直すと通知が届きます。  
仕組みの違いまでは追えていませんが、当時もこれに気づいて替えたのだろうと思います。  
通知はこの使い方の柱なので、いま選び直すとしてもghosttyにします。

ghostty自体は過去にも一度試して、そのときはalacritty + tmuxに戻っています。  
ターミナルはiTerm2、wezterm、warpも使ってきました。  
iTerm2は完成度が高い一方、設定がバイナリ形式でgitでの管理がしっくりきませんでした。  
warpは日本語変換に難があり、weztermは良い点もあったもののどこがとは言えないまま合わず、それぞれ定着しませんでした。  
herdrとの組み合わせになって、ようやくghosttyに落ち着いた形です。

## 失ったもの

ほとんどありません。  
alacrittyは速さが売りですが、ghostty + herdrにして遅いと感じたことはないです。

強いて挙げると2つあります。  
tmuxのcopy-mode相当の画面で、`0`や`$`での行頭・行末ジャンプと`w`や`b`での単語移動の挙動が今ひとつです。  
それと、tmuxでペインに番号を出して選ぶ機能（display-panes）に相当するものがありません。  
ただ、前述の型でペインの数自体が減ったので、後者は困らなくなりました。

## ハマりどころ2件（どちらも日本語入力）

### 日本語が打てなくなる

乗り換えてしばらくして、ターミナルでだけ日本語が打てなくなる現象に当たりました。  
...
prefixキー（操作の合図として最初に押すキー）を使っている間だけ入力ソースをASCIIに切り替える機能で、日本語IMEのままprefixを押して文字が化けるのを防いでくれるのですが、元の入力ソースに戻し損ねることがあります（[herdr #1221](https://github.com/herdrdev/herdr/issues/1221)、0.7.3時点で未修正）。  
...
herdrを一言でいうと、tmuxをAIエージェント向けに作り直したもの、という説明が一番近いと思います。  
tmuxでできていたことはそのままに、エージェントの状態が見えることと通知が来ることが足されました。  
私の場合は、repo=space・タスク=タブ・1タスク1エージェントという型が決まったことと、巡回が通知に置き換わったことが一番の変化です。  
エージェントを複数走らせる使い方をしているなら、いまの端末のまま試せます。  
ただし通知の出方は端末によって差があります。手元ではalacrittyだとデスクトップ通知が出ませんでした。  
一度触ってみてください。
