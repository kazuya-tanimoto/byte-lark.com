...
- **PHASE1C-004（ロゴ）**: 完了（2026-08-01）。着手時の指示は「sky `#0273B0` 基調 + sun `#F0A32A`」だったが、**確定意匠は墨一色（`currentColor`）**。理由は 2 つ——小サイズのマーク単体で Twitter 旧ロゴに似るのを避ける防波堤として墨一色を選んだこと、`currentColor` ならダークモードで自動反転すること（経緯は PBI 実装ログ ラウンド 7）。トーン「軽やか・上昇（揚雲雀）」は不変。アセットは logo.svg（フルマーク・大サイズ用）/ logo-badge.svg（ヘッダー 26px 用バッジ）/ logo-bird.svg（鳥単体）
- **PHASE1C-005（favicon）**: 完了（2026-08-01）。**sky `#0273B0` タイル + 白抜きバッジ**。ロゴ本体の墨一色とは別扱いにした——タブやホーム画面ではサイトの地の色が使えず、16px で並ぶタブの中から見つけてもらうには塗りタイルの方が強いため（比較モックは `docs/design-drafts/phase1c-005/favicon-candidates.html`）。形の語彙（冠羽・上昇姿勢・切れ目リング）は logo-badge.svg のまま
