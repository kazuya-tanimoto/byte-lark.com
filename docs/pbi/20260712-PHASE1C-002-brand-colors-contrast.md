...
- [ ] `tests/e2e/a11y.spec.ts` の `disableRules(["color-contrast"])`（line 26-28 付近）を解除し、axe が全対象ページで green（解除済み・ローカル iframe axe で全 8 ページ green。CI green は push 後確認）
...
- [ ] Lighthouse Accessibility で color-contrast 監査 pass、主要ページ 90+ 維持（サンドボックスで Chrome 起動不可のため運営者ターミナル実行。PHASE1A-020 と同じ分担）
