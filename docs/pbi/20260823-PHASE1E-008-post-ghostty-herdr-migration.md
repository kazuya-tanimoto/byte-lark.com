...
### 2026-09-12 セッション 4 続き 3（display-panes 画像と文面の反映）
- display-panes 節の文面を運営者が承認 → 反映。tmux のスクショ（チャット添付）を
  コンテナの画像キャッシュから `tmux-display-panes.png` として記事フォルダに配置
  - 前回「リポジトリへの配置をお願いします」と依頼したのは誤り。添付画像は
    `~/.claude/image-cache/<session>/` に実体があり、コンテナから配置できる。以後は自分でやる
- herdr 0.9.0 のスクショ 2 枚（統合版 = 全体、サイドバー単体）をチャットで受領。
  差し替え先の提案と赤枠版の説明文面を運営者に提示し、OK 待ち
- 検査：natural-japanese lint --genre tech 0 件（84 文）、profile「避ける表現」照合済み

### 2026-09-12 セッション 4 続き 4（herdr 画像 3 枚の反映とプレビュー送付）
- 運営者が「文面は一旦OK。差し替え方針も提案でやってみましょう。出来上がり見ないと
  なんとも言えない」と回答 → 実施して出来上がりを送付する方式に
- 枠あり版（運営者がサイドバー単体に赤枠 + 注釈を書き込んだもの）を「herdrとは」節に
  掲載。承認文面は統合版ベースの想定だったため、導入文と alt を実際の画像に合わせて
  調整（herdr-sidebar-annotated.png。調整は運営者に報告）
- 統合版で herdr-overview.png を差し替え。alt の「この記事を書いているセッション自体が
  写っている」は、新画像の中央が別プロジェクトのため削除
- サイドバー単体から agents 部分を切り抜き（411x295）herdr-agents-sidebar.png を差し替え
- draft を一時 false にしてローカル dev でフルページスクショを撮影し運営者に送付。
  撮影後 draft: true へ戻し、一時スクリプト（scripts/tmp-shot-preview.mjs）は削除済み
- 検査：natural-japanese lint --genre tech 0 件（86 文）、profile「避ける表現」照合済み
...
### 2026-09-13 セッション 4 続き 8（設定抜粋の差し替えとレビュー 2 回目）
- 設定抜粋のコメント 4 行を dotfiles の実物に合わせた（運営者「当然直す」。7d38364）
- /article-review 2 回目を subagent で実施（総評「公開可能に近い」、lint 0 件、
  指摘 16 件 + 運営者向け確認 3 件。全文は scratchpad/article-review-2.md）
- 想定外：指摘を仕分けずに 16 件と確認 3 件をそのまま運営者に渡し、
  「本文と見比べないと答えられない」「細かすぎる」と指摘を受けた。
  原因：(1) §9 に Claude の仕分け工程が無く、§8「OK が出た項目だけ反映」を
  誤字レベルにも当てはめた (2) レビュー担当に運営者リライト文を「運営者の文」と
  伝えず、言い回しの指摘が混ざった (3) 質問で文を引用せず行番号で指した
  (4) 「設定は既定でオンか」をヒアリング・ドラフト・1 回目レビューのどこでも
  確かめていなかった（取材メモに既定値の記述なし）。
  対応：決まりの修正（§9 仕分け工程、article-review スキル、§5 深掘りの目安）を
  運営者に提案中。文面は OK 後に反映
- 運営者承認で反映（4 箇所）：`switch_ascii_input_source_in_prefix` は既定 false の
  opt-in（v0.6.8 リリースノート「opt-in」、0.9.0 Config reference default false）
  のため「この設定は既定では無効で、私はAIの提案を受けて有効にしていました。」を追加
  （運営者回答「AI の提案のはず」）／64 行目の行末スペース／69 行目「ウィンドウ」→
  「ウインドウ」／スター数 37,000 → 38,000（GitHub API 38,019）
- 不採用 10 件（運営者リライト文への言い回し・体裁の提案。記事に入れない）と、
  運営者に聞かなかった確認 3 件（操作の書き方・「化ける」の表現・T9 リンク）。
  T9 リンクは devcontainer の 1 文削除（続き 5）の帰結として張らない
- 画像の写り込み（ホスト名・monotrip.jp・コスト表示）：続き 4 の「統合版の写り込み
  （monotrip.jp）の最終可否は OK」を画像全体の承認として扱う
- 公開前の再確認：herdr 最新 v0.9.0（2026-09-07）、ghostty 1.4.0 未リリース
  （最新タグ v1.3.1）、PR #12547 は v1.3.1 に未収録（compare API: ahead）
