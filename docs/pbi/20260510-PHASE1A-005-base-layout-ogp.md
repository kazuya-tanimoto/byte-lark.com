...
## 受け入れ条件
- [ ] `src/layouts/BaseLayout.astro` を作成（props: title / description / ogImage / canonical / type）
- [ ] `<title>` / `<meta name="description">` が各ページから渡された値で出力
- [ ] OGP メタ（og:title / og:description / og:image / og:url / og:type）が SSG 出力 HTML に静的に含まれる
- [ ] Twitter Card メタ（`summary_large_image`）が出力
- [ ] `<link rel="canonical">` が各ページの正規 URL で出力
- [ ] デフォルト OGP 画像 `public/og-default.png` を配置（ogImage 未指定時のフォールバック）
- [ ] `src/layouts/PageLayout.astro` を作成（一般ページ用、BaseLayout を wrap）
- [ ] OGP ヘルパ `src/lib/og.ts` を作成（メタタグ生成ロジックの集約）
- [ ] `yarn build` 成功、出力 HTML で OGP メタを目視確認
- [ ] `yarn check:ts` エラーなし
