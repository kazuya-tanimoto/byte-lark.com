...
- **本文フォント（optional）は本番でも実際には使われていない**。本文の woff2 だけ落とせないようにして撮り比べると差分 0px（見出しは 2910〜18114px の差＝効いている）。変更前の本番も同じなので今回の退行ではないが、258KB を落として画面に反映されていないことになる。→ 申し送り
