...
### 2026-08-08 事後追記：ダークモード関連 3 件は破棄ではなく申し送り

棚卸し表の「ダークモードの実表示検証」行は、判定を「持ち越し（やらないと決めた時点で破棄）」と書いていた。PHASE1D-001 で実表示を確認して見送りが確定したが、運営者判断により 3 件（`.dark` トークン実表示 / currentColor アセット / favicon の sky 固定）は破棄せず次 Phase への申し送りとして残す。

破棄しない理由は、PHASE1D-001 で破綻の原因と実測コントラストまで押さえたため。再着手時にこの調査をやり直さずに済む。詳細は PHASE1D-001 の「決定事項」「次 Phase への申し送り」を参照。
