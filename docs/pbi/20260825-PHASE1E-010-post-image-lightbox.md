...
- [ ] 拡大時に十分な解像度で表示される（現状の出力は webp 変換のみで元解像度のまま。
      同じ src の拡大で足りることを実表示で確認し、将来 `widths` / `sizes` で縮小版を
      作る変更が入ったら再考、と実装ログに記録）
...
- 実装方針（推奨）: `PostLayout.astro` に `<dialog>` 1 個 + 数十行の vanilla script を置き、
  ...
  「依存なし・1 コンポーネント完結」の作り。拡大画像は同じ src を `max-width: 100vw` 系で
  表示し、解像度が足りない場合のみ元ファイル参照を検討
