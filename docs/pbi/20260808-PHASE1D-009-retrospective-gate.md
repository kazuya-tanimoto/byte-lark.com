...
### 次セッションへのトリガー
- [x] **PHASE1E-001（公開後の小さな手入れ 6 件）を本セッション内で起票済み**（`20260810-PHASE1E-001-postlaunch-housekeeping.md`、Status: NotStarted）。次セッションはその着手から。当初は「次セッションで起票」としていたが、運営者指摘で引き継ぎの導線に穴があることが分かり、実ファイルとして起票する形に変えた（下の実装ログ「引き継ぎの導線を実ファイルに変えた」参照）
...
| 本 Gate | `docs/pbi/INDEX.md`（436 行 / 80KB）と `docs/site-plan.md`（630 行 / 86KB）が `Read` の 1 回分を超えており、セッション開始時に読む位置に現在地が出てこない | 持ち越し（本 Gate では INDEX 冒頭に「次にやること」節を置いて当座を塞いだ。根治は改訂履歴の切り出しか Phase 別ファイルへの分割で、別 PBI が要る） |
...
#### 引き継ぎの導線を実ファイルに変えた（運営者指摘）

Gate を Done にした後、運営者から「次セッションで間違いなく起票できる仕組みになっているか」と問われて確かめたところ、なっていなかった。

- `docs/pbi/INDEX.md` は 436 行 / 80KB、`docs/site-plan.md` は 630 行 / 86KB。どちらも `Read` の 1 回分（約 274 行で打ち切り）を超えており、**申し送りを書いた場所がすべて切れた先にあった**——INDEX の Phase 1e 節は 340 行、改訂履歴は 367 行、site-plan §12 次アクションは 530 行
- INDEX に置いた `PHASE1E-001 … NotStarted` の行には対応するファイルが無く、セッション開始時の必須チェック（`docs/pbi/*PHASE*.md` を grep する方式）の**検出対象にすらならなかった**。書き漏らしても警告が出ない

つまり「Gate の申し送りを読む」導線は、先に「次は Phase 1e」と気づいている必要があり、その気づき自体が読めない位置にある循環になっていた。2 つ入れて塞いだ。

1. **PHASE1E-001 を実ファイルとして起票**（`20260810-PHASE1E-001-postlaunch-housekeeping.md`）。ファイルがあれば開始時チェックの grep に載り、§7 検証 3 行の常設チェックも効く。INDEX を読み切れなくても PBI 選択が成立する
2. **INDEX 冒頭に「次にやること」節**を新設（現在地 / 次の PBI / ブランチ運用 / 直前 Gate の申し送りへのリンク）。ファイル先頭なので必ず読まれる。あわせて「このファイルは 1 回で読み切れない」ことを本文に明記した

根治は別（改訂履歴が本体を押し出しているので、切り出しか Phase 別分割が要る）。棚卸し表に持ち越しとして記録した。

学び：申し送りは「書いたかどうか」ではなく「**次に読む人の目に入る位置か**」で見る。ファイルが読める量を超えていることに、書いている側は気づけない。
