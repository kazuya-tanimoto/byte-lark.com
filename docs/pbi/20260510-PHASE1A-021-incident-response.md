...
## 受け入れ条件
- [x] `docs/incident-response.md` を作成
- [x] 監視方法の記載（Cloudflare セキュリティイベント / UptimeRobot 等の選択肢）（§2）
- [x] 漏えい・改ざん検知時の対応手順（初動 → 調査 → 復旧 → 報告）（§3 共通フロー + §6 ケース別）
- [x] 対応者・連絡先の明記（法人化前は運営者本人）（§1）
- [x] Cloudflare ダッシュボードでのセキュリティイベント確認手順の概要（§5）
