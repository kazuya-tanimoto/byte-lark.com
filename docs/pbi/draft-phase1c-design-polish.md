...
Status: Draft（番号なし。Phase 1b 完了 + Phase 1a Gate 通過後の 1c 起票セッションで番号付き PBI として正式化する）
...
## 1c 着手条件
- [ ] Phase 1b（コンテンツ整備）完了
- [ ] Phase 1a Gate（PHASE1A-022）通過

## A. site-plan に既出のデザイン確定項目（再掲・参照のみ）
- 仮 HEX → 確定 HEX 置換（site-plan §6.5.2）
- **color-contrast 再有効化**：確定 HEX で AA 4.5:1 を満たす値を選定し、`tests/e2e/a11y.spec.ts` の `disableRules(["color-contrast"])` 除外を解除（site-plan §6.5.2 a11y 追跡 / PHASE1A-019 起点）。Lighthouse でも color-contrast が全ページで唯一の A11y 失敗監査（PHASE1A-020 で確認）
- タイポスケール・行間・和欧混植の確定（site-plan §6.5、TODO 化済み）
- ロゴ刷新：ClaudeDesign でヒバリ意匠 + ブランドカラー、反復上限 5 ラウンド（site-plan Decision #15 / R-06 / Q11）
...
（正式 PBI 化のとき、上記を個別 PBI に分解し、各 PBI に CLAUDE.md §7 検証ゲート 2 項目を受け入れ条件として常設する。README §4.6 ルール 7）
