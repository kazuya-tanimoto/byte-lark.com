...
| 2026-08-25 | v3.17：**「先頭へ戻る」ボタンの本番不具合修正**（PHASE1E-009 追修正）。出現しきい値を「1 画面ぶん」→ 固定 300px（/contact は 1512×982 で可動域 527px しかなく一度も出なかった）、追従目次の判定を「その画面幅で描画されるか（offsetParent）」→「いま画面内に見えているか（getBoundingClientRect）」（高さ 280px の窓では記事末尾で目次が画面外へ抜け、ボタンも出ず先頭へ戻る手段が消えていた）。Decision #33 を追修正（却下案としていた画面内判定を採用に変更）。CLAUDE.md（Related Docs 版数）連動 |
| 2026-09-05 | v3.18：**devcontainer の住み分け記録を実運用に合わせた**（Decision #34 追記）。コンテナ内セッションのスクショ確認は `scripts/capture-screenshots.mjs`（コンテナ内 headless Chromium）、MCP Playwright は母艦のみ、という 2026-08-09（PHASE1D-012）以降の運用を devcontainer-plan §1.3-3 / §7、CLAUDE.md、pbi-review skill、Stop hook プロンプト、スクリプト冒頭コメントに反映。運用自体の変更はなし |
